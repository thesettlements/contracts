...
contract SettlementsV2 is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    address private legacySettlementAddress;

    constructor(address _legacyAddress) ERC721("Settlements", "STL") {
        legacySettlementAddress = _legacyAddress;
    }
    ...
    string[] public _sizes = ['Camp', 'Hamlet', 'Village', 'Town', 'District', 'Precinct', 'Capitol', 'State'];
    string[] public _spirits = ['Earth', 'Fire', 'Water', 'Air', 'Astral'];
    string[] public _ages = ['Ancient', 'Classical', 'Medieval', 'Renaissance', 'Industrial', 'Modern', 'Information', 'Future'];
    string[] public _resources = ['Iron', 'Gold', 'Silver', 'Wood', 'Wool', 'Water', 'Grass', 'Grain'];
    string[] public _morales = ['Expectant', 'Enlightened', 'Dismissive', 'Unhappy', 'Happy', 'Undecided', 'Warring', 'Scared', 'Unruly', 'Anarchist'];
    string[] public _governments = ['Democracy', 'Communism', 'Socialism', 'Oligarchy', 'Aristocracy', 'Monarchy', 'Theocracy', 'Colonialism', 'Dictatorship'];
    ...
    function claim(
        ...
        SettlementsLegacy(legacySettlementAddress).transferFrom(msg.sender, address(this), tokenId);
        ...
        bytes32 legacyURI = keccak256(abi.encodePacked(SettlementsLegacy(legacySettlementAddress).tokenURI(tokenId)));
